...
mainp0:
	...
	ori $t6, $zero, 100 	#to check if the first element is added
	ori $t8, $zero, 100	#save the total number of times,i.e the counter
...
for1:	ori $a0, $zero, 0xabf2 #0x30 is the seed
...
incr_h:	#now we will assign $t5 value to head
	...
	ori $a0, $zero, lock_head
	jal unlock
	addi $t8, $t8, -1 #decrease the count value by 1
...
incr_t:	#now we will assign $t1 value to tail
	...
	addi $t0, $t0, 4
	beq $t0, $t9, tail0
new_t:	ori $t1, $zero, tail
	sw $t0, 0($t1)
	#critical code segment end
	ori $a0, $zero, lock_tail
	jal unlock
	j eval
tail0:	ori $t0, $zero, 0x1000
	j new_t
