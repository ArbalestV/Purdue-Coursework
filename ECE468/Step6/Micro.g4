...
var_decl 
	 : var_type id_list ';' {createSymbolTable.insertVariable($var_type.text, $id_list.text, null); AST.addToFunctionParameterList($id_list.text, "L");}
	 ;
...
func_decl 
	  : 'FUNCTION' any_type id {/*System.out.println("In new function");*/ createSymbolTable.FunctionScopeTable($id.text); AST.createFunction($id.text); createSymbolTable.numParams = 0; createSymbolTable.insideFunction = 1;} '('param_decl_list ')' {/*System.out.println("Number of paramaters in this function: " + createSymbolTable.numParams);*/} 'BEGIN' func_body 'END' {AST.addFunctionSymbolTable(); createSymbolTable.popSymbolTable(); createSymbolTable.insideFunction = 0;} 
	  ;
