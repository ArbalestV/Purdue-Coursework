...
void main (int argc, char * argv[])
{
  ...
  cond_t cond_handler_prod;// I changed this for q4 to be a cond_t
  cond_t cond_handler_cons;// handler for consumers
  lock_t lock_handler;
  char cond_handler_prod_str[10];//cond now
  char cond_handler_cons_str[10];
...
/* //wait why is this here? this just repeats what is up above? 
  ...
  cond_handler_prod = cond_create(lock_handler);//then use lock to create cond
  cond_handler_cons = cond_create(lock_handler);//create cv for the consumer
  ...
  ditoa(cond_handler_prod, cond_handler_prod_str);
  ditoa(cond_handler_cons,cond_handler_cons_str);
  ...
  for(i=0; i<numprocs; i++) {
    process_create(PRODUCER_TO_RUN, h_mem_str, s_procs_completed_str,lock_handler_str, cond_handler_prod_str, cond_handler_cons_str, NULL);
    Printf("Producer %d created\n", i);
    process_create(CONSUMER_TO_RUN, h_mem_str, s_procs_completed_str,lock_handler_str, cond_handler_prod_str, cond_handler_cons_str, NULL);
