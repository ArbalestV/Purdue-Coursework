...
//	CondInit
//
//	Initialize a condition variable to a particular value.  This just means
...
cond_t CondCreate(lock_t lock) {
  ...
  cond_t c;
  uint32 intrval;
  if (lock == INVALID_LOCK) { //check if the lock is valid
    printf("FATAL ERROR: Lock is invalid so Con Variable not created.\n");
    //return SYNC_FAIL;
    return INVALID_COND;
  }
  else {//if the lock is valid then proceed to create the cv
    // grabbing a lock should be an atomic operation
    intrval = DisableIntrs();
    for(c=0; c<MAX_CONDS; c++) {
      if(conds[c].inuse==0) {
	conds[c].inuse = 1;
	conds[c].lock_handle = lock;
	break;
      }
    }
    RestoreIntrs(intrval);
...
int CondWait(Cond * cond){
  ...
  int lock_reacquire;
  intrval = DisableIntrs();
  lock_h = cond->lock_handle;
  if (GetCurrentPid() != (locks[lock_h].pid)) {
    return SYNC_FAIL; //basically fail if the current process does not have the lock
  }
  
  lock_return = LockHandleRelease(lock_h); 
  if (lock_return == SYNC_FAIL){
    printf("Somehow not being able to release a lock for a cv.\n");
    return SYNC_FAIL;//1 not0!!
  }
  else{
    if ((l = AQueueAllocLink ((void *)currentPCB)) == NULL) {
      printf("FATAL ERROR: could not allocate link for cond variable queue in SemWait!\n");
      exitsim();
    }
    if (AQueueInsertLast (&cond->waiting, l) != QUEUE_SUCCESS) {
      printf("FATAL ERROR: could not insert new link into cond variable waiting queue in SemWait!\n");
      exitsim();
    }
    ProcessSleep();
    //might need to move this section to the user process
    lock_reacquire = LockHandleAcquire(lock_h);
    if (lock_reacquire == SYNC_FAIL){
      printf("Could not re-acquire lock for a cv.\n");
      RestoreIntrs(intrval);
      return SYNC_FAIL;//1 not0!!
    }
    RestoreIntrs(intrval);
...
int CondHandleWait(cond_t c) {
  if(c < 0) return SYNC_FAIL;//1 not syncfail
  if(c >= MAX_CONDS) return SYNC_FAIL;//1 not sync fail
  if(!conds[c].inuse) return SYNC_FAIL; //1 not sync fail
  return CondWait(&conds[c]);//0 nt sync success
}
...
int CondSignal (Cond * cond) {
  ...
  if (!AQueueEmpty(&cond->waiting)) { //if there is a process to wake up
    l = AQueueFirst(&cond->waiting);
    pcb = (PCB *)AQueueObject(l);
    if (AQueueRemove(&l) != QUEUE_SUCCESS) {
      printf("FATAL ERROR: could not remove link from cond variable queue in CondSignal!\n");
      exitsim();
    }
...
int CondHandleSignal(cond_t c) {
  ...
  if(c >= MAX_CONDS) return SYNC_FAIL;//1 not sync fail
  if(!conds[c].inuse) return SYNC_FAIL; //1 not sync fail
  return CondSignal(&conds[c]);//0 nt sync success
}
...
int CondBroadcast (Cond * cond) {
  ...
  while (!AQueueEmpty(&cond->waiting)) { //as long as there is a process to wake up
    l = AQueueFirst(&cond->waiting);
    pcb = (PCB *)AQueueObject(l);
    if (AQueueRemove(&l) != QUEUE_SUCCESS) {
      printf("FATAL ERROR: could not remove link from cond variable queue in CondSignal!\n");
...
int CondHandleBroadcast(cond_t c) {
  ...
  if(c >= MAX_CONDS) return SYNC_FAIL;//1 not sync fail
  if(!conds[c].inuse) return SYNC_FAIL; //1 not sync fail
  return CondBroadcast(&conds[c]);//0 nt sync success
}
