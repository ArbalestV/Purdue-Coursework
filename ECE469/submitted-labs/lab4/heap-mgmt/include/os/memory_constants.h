...
#define MEM_L1FIELD_FIRST_BITNUM 12 //meaning 0xc
...
#define MEM_PAGESIZE (0x1 << MEM_L1FIELD_FIRST_BITNUM) //makes sense -> size of each page denoted by how big the page offset is 100000000000 = 2^12
#define MEM_L1TABLE_SIZE ((MEM_MAX_VIRTUAL_ADDRESS + 1) >> MEM_L1FIELD_FIRST_BITNUM) //2^22 >> 12 = 2^(22-12) = 2^10 - size of the page table per process
#define MEM_ADDRESS_OFFSET_MASK (MEM_PAGESIZE - 1) //0 to MEM_PAGESIZE

#define MEM_MAX_PHYS_MEM (0x1 << 21) //since 2MB
#define MEM_MAX_PAGES (MEM_MAX_PHYS_MEM / MEM_PAGESIZE) //total available physical memory / size of each page
