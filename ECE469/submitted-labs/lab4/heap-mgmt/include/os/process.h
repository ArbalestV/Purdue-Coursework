...
typedef struct PCB {
  ...
  uint32	pagetable[MEM_L1TABLE_SIZE]; // Statically allocated page table - each process in its page table will have MEM_L1TABLE_SIZE no of entries, all stored as an integer array
  Link		*l;		// Used for keeping PCB in queues
  Queue heap_info;
  heap_node heap_node_get[100]; //this is a trick to get the available heapnodes. 100 is chosen arbitrarily
} PCB;
