...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    for (j = 0; j < MEM_L1TABLE_SIZE; j++)
      {
	//printf("\ni = :%d", i);
	pcbs[i].pagetable[j] = 0; //store 0's in every element of the Page Table
      }

    //now we will just initialize all the heap_node_get.isOccupied to 0
    for (j = 0; j < 100; j++)
      {
	pcbs[i].heap_node_get[j].isOccupied = 0;
      }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //heap queue initialize
  initial_index = getFreeBlock(pcb); //get the initial index
  if (initial_index == MEM_FAIL) //just a check. would not be used during initialization
    {
      printf("\nERROR: Could not get any free block to initialize with.\n");
      exitsim();
    }
  pcb->heap_node_get[initial_index].start_addr = (void *) (4 * MEM_PAGESIZE); //since we already know that it will begin from the 5th page in the page table of the process.
  pcb->heap_node_get[initial_index].isOccupied = 0; //dont need to do this, but just for clarity's sake
  pcb->heap_node_get[initial_index].size = MEM_PAGESIZE; //the entire 4KB of page is assigned. We will eventually break chunks off of this.
  if (AQueueInit(&(pcb->heap_info)) == QUEUE_FAIL) //just a check
    {
      printf("\nERROR: Malloc Queue could not be initialized.\n");
      exitsim();
    }
  //queue has been initialized
  first_queue_element = AQueueAllocLink(&(pcb->heap_node_get[initial_index]));
  //now put the first queue link into the initialized queue
  AQueueInsertFirst(&(pcb->heap_info), first_queue_element);
  //end of new stuff for heap management

  
  stackframe = (uint32 *) (pcb->sysStackArea + 0xffc); //last address of the sysStack PTE - made it 4 byte format
  //printf("\nprinting the stack frame address : %x\n", (uint32) stackframe); //this is working fine. stackframe will itself have value in hex but will be represented as a 32-bit integer
  ...
  stackframe[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM
				      + (MEM_L1FIELD_FIRST_BITNUM << 16)); //just copied from previous lab -> ask TA what this means -> for L1 PT, they are same
