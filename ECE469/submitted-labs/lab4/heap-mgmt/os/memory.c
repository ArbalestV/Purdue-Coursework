...
void MemoryModuleInit() {
  ...
  //os_reserved_pages = 90; //just to compare with 2-level
...
uint32 MemorySetupPte (uint32 page) {
  //return -1;
  page = page << MEM_L1FIELD_FIRST_BITNUM; //since 12th bit onwards actual physical addresses begin
...
void MemoryFreePage(uint32 page) { //function to free a physical memory page, i.e., set the corresponding freemap to 1
  ...
  page = page >> MEM_L1FIELD_FIRST_BITNUM; //since 12th bit onwards actual physical addresses begin -> reverse of the previous function's operation
...
void * malloc(PCB * pcb, int memsize)
{
  ...
  int min_size;
  int remainder;
  int fragmented_size; //this will only be used for fragmentation
  Link * smallest;
  Link * l;
  heap_node * current_heap_node; //the actual heap node during iterations
  heap_node * to_be_used_heap_node; //this will store the one we will end up using to maklloc, and will external fragment out of it required.
  
  uint32 physical_addr;
  int new_block_index;
  Link * newlink;

  min_size = MEM_PAGESIZE; //since this is the maximum size of a chunk of heap memory
  remainder = memsize % 4;
   l = AQueueFirst(&(pcb->heap_info)); //get the first element of the queue to iterate over
  smallest = NULL; //this will be used to check if any block fulfilling our criteria has been found yet
  current_heap_node = AQueueObject(l); //for comparison
  if (memsize <= 0)
    {
      printf("\nMemory to be allocated cannot be less than or equal to 0.\n");
      return NULL;
    }
  if ( remainder == 0)
    {
    ...
    {
      req_size = memsize + (4 - remainder);
    }

  while (l != NULL)
    {
      if (current_heap_node->isOccupied == 0 && current_heap_node->size >= req_size && current_heap_node->size <= min_size)
	{
	  smallest = l;
	  min_size = current_heap_node->size;
	}
      l = AQueueNext(l);
      current_heap_node = AQueueObject(l);
    }

  if (smallest == NULL) //essentially a big enough block was not found
    {
      printf("\nERROR: No large enough memory chunk exists.\n");
      return NULL;
    }
  //now, we have a memory chunk. Now compare if we have fragmentation or not. Accordingly we will have to perform additional operations if there is external fragmentation.
  to_be_used_heap_node = AQueueObject(smallest); //since this will be the smallest chunk acc to best fit algorithm
  if (req_size == to_be_used_heap_node->size) //no external fragmentation
    {
      fragmented_size = 0;
      to_be_used_heap_node->isOccupied = 1; //signify it is currently being held
      physical_addr = pcb->pagetable[4] & 0x3FF000; //to mask out the status bits
      physical_addr = physical_addr + ((uint32) to_be_used_heap_node->start_addr & 0xFFF); //now added the virtual address by masking off the page offset of the virtual address
      //now the print statement
      printf("\nCreated a heap block of size %d bytes: virtual address 0x%x, physical address 0x%x\n", to_be_used_heap_node->size, to_be_used_heap_node->start_addr, physical_addr);
      return to_be_used_heap_node->start_addr; //return the virtual address
    }


  //external fragmentation case
  to_be_used_heap_node->isOccupied = 1; //signify it is currently being held

  fragmented_size = to_be_used_heap_node->size - req_size;
  to_be_used_heap_node->size = req_size; //the block size will only be the requisite size
  //now we will get a new block and make it the next of this allocated block
  new_block_index = getFreeBlock(pcb);
  pcb->heap_node_get[new_block_index].isOccupied = 0;
  pcb->heap_node_get[new_block_index].size = fragmented_size;
  pcb->heap_node_get[new_block_index].start_addr = (void *) ((uint32) to_be_used_heap_node->start_addr + req_size);
  
  newlink = AQueueAllocLink(&(pcb->heap_node_get[new_block_index]));
  //now insert l after the smallest block
  AQueueInsertAfter(&(pcb->heap_info), smallest, newlink);
  
  //now the same calculation and return statement as no fragmentation case
  physical_addr = pcb->pagetable[4] & 0x3FF000; //to mask out the status bits
  physical_addr = physical_addr + ((uint32) to_be_used_heap_node->start_addr & 0xFFF); //now added the virtual address by masking off the page offset of the virtual address
  //now the print statement
  printf("\nCreated a heap block of size %d bytes: virtual address 0x%x, physical address 0x%x\n", to_be_used_heap_node->size, to_be_used_heap_node->start_addr, physical_addr);
...
{
  Link * l; //the iterator
  heap_node * found_block;
  heap_node * prev_node;
  Link * prev;
  heap_node * next_node;
  Link * next;
  int bytes_freed; //the return value
  uint32 physical_addr;
  
  l = AQueueFirst(&(pcb->heap_info)); //start from the very first element
  found_block = AQueueObject(l);

  if (ptr == NULL)
    {
      printf("\nError: The requested Virtual Address cannot be NULL.\n");
      return MEM_FAIL;
    }

  while (l != NULL)
    {
      if (found_block->start_addr == ptr)
	{
	  bytes_freed = found_block->size;
	  physical_addr = pcb->pagetable[4] & 0x3FF000; //to mask out the status bits
	  physical_addr = physical_addr + ((uint32) found_block->start_addr & 0xFFF); //now added the virtual address by masking off the page offset of the virtual address
	  found_block->isOccupied = 0; //to signify it is no longer occupied
	  break;
	}
      //next iteration
      l = AQueueNext(l);
  ...
  if (l == NULL)
    {
      printf("\nError: The requested block to be freed does not exist in the heap.\n");
      return MEM_FAIL;
    }
  //put the print statement
  printf("\nFreeing heap block of size %d bytes: virtual address 0x%x, physical address 0x%x\n", bytes_freed, found_block->start_addr, physical_addr);
  //now check for previous and next blocks to see if they are occupied or not, and whether merging is required.
  //now look into the previous block
  prev = AQueuePrev(l);
  prev_node = AQueueObject(prev);
  if (prev_node->isOccupied == 0)
    {
      //then merge the previous with the found
      found_block->size = found_block->size + prev_node->size;
      found_block->start_addr = prev_node->start_addr;
      //now remove the previous node
      AQueueRemove(&prev);
    }

  //now do the same for next block
  next = AQueueNext(l);
  next_node = AQueueObject(next);
  if (next_node->isOccupied == 0)
    {
      //then merge the previous with the found
      found_block->size = found_block->size + next_node->size;
      //found_block->start_addr = prev_node->start_addr;
      //now remove the previous node
      AQueueRemove(&next);
    }
...
int getFreeBlock(PCB * pcb)
{
  ...
  for (i = 0; i < 100; i++)
    {
      if (pcb->heap_node_get[i].isOccupied == 0)
	{
	  //pcb->heap_node_get[i].isOccupied = 1;
	  return i; //get the first unoccupied one
	}
    }
  return MEM_FAIL;
}
