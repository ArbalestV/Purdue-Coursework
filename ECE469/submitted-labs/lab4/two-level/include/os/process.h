...
typedef struct PCB {
  ...
  //l2_pagetable * pagetable[MEM_L1TABLE_SIZE]; //this is the array that will store the entries to the 4 Page Tables, each storing a physical address in memory 
  //uint32	pagetable[MEM_L2TABLE_SIZE]; // Statically allocated page table - each process in its page table will have MEM_L2TABLE_SIZE no of entries, all stored as an integer array
