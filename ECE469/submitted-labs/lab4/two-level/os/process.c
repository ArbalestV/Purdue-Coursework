...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    //printf("\nSize of the MEM_LITABLE_SIZE : %d", MEM_L1TABLE_SIZE);
    for (j = 0; j < MEM_L1TABLE_SIZE; j++)
      {
	//printf("\ni = :%d", i);
	pcbs[i].pagetable[j] = NULL; //store 0's in every element of the Page Table
      }
...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  //basically all PTEs with valid bit set call MemoryFreePage() on them. Ask the GTA whether or not to call MemFreePage() on system  - TA said yes
  for (i = 0; i < MEM_L1TABLE_SIZE; i++)
    {
      /*comp_val = pcb->pagetable[i] & MEM_PTE_VALID;
      if (comp_val > 0)
	{
	  //free this PTE
	  pcb->pagetable[i] = pcb->pagetable[i] ^ MEM_PTE_VALID; //make the valid bit 0
	  MemoryFreePage(pcb->pagetable[i]); //free the page
	  }*/
      if (pcb->pagetable[i] != NULL)
	{
	  addr = pcb->pagetable[i];
	  *(addr - 4) = 0;
	  for (j = 0; j < MEM_L2TABLE_SIZE; j++)
	    {
	      //comp_val = addr->table[j] & MEM_PTE_VALID;
	      comp_val = addr[j] & MEM_PTE_VALID;
	      if (comp_val > 0)
		{
		  addr[j] = addr[j] ^ MEM_PTE_VALID; //make the valid bit 0
		  MemoryFreePage(addr[j]);
		}
	    }
	  //addr->inuse = 0; //make it back to 0 signifying it is free
	  pcb->pagetable[i] = NULL;
	}
    }

  MemoryFreePage(pcb->sysStackArea); //free the system stack frame for this process
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  int j; //for iterating though l2[]
  //l2_pagetable * address;
  uint32 * address;
  ...
  if ((pcb->sysStackArea = MemoryAllocPage()) == MEM_FAIL) //page could not be allocated 
    {
      //printf("\nFATAL ERROR : Page could not be allocated for system stack.\n");
      exitsim();
    } //have allocated a new page for pcb->sysStackArea
  
  else //page could be allocated 
    {
      pcb->sysStackArea = MemorySetupPte(pcb->sysStackArea);
      pcb->sysStackArea = pcb->sysStackArea - 1; //remove the valid bit
    }
  //printf("\nSystem Stack Area value: %x\n", pcb->sysStackArea);
  //printf("\nfinished allocating system stack.\n");
  //allocate 4 pages - the first 4, 0-3
  /*for(i = 0; i < 4; i++)
    {
      //printf("\nallocating for page # : %d\n", i);
      if ((pcb->pagetable[i] = MemoryAllocPage()) == MEM_FAIL) //page could not be allocated 
	{
      ...
      else //page could be allocated 
	{
      ...
      //printf("\nfinished allocating for page # : %d\n", i);
    }*/
  //now go to the very first entry of the pagetable, and set it to the very first element of l2[] array that is not in use
  for (j = 0; j < 256; j++)
    {
      if (l2[j].inuse == 0)
	{
	  l2[j].inuse = 1;
	  //printf("\nSelected this: %d\n", j);
	  //printf("\nGetting the l2+j value : %x\n", &(l2[j]));
	  //printf("\nl2[j].table: %x\n", l2[j].table);
	  //pcb->pagetable[0] = &(l2[j]); //the address
	  pcb->pagetable[0] = l2[j].table;
	  break;
	}
    }

  //now within the address pointed to by the pagetable[0] entry, allocate four pages
  for (i = 0; i < 4; i++)
    {
      address = pcb->pagetable[0];
      //printf("\nAddress at p[0]: %x\n", address);
      //printf("\nBefore allocating for one of the four pages...\n");
      //printf("\naddress[i]: %x\n", address[i]);
      if ((address[i] = MemoryAllocPage()) == MEM_FAIL) //page could not be allocated 
	{
	  //printf("\nFATAL ERROR : Page could not be allocated.\n");
	  exitsim();
	}
	...
	{
	  //printf("\nBefore calling setupPTE() function value = %x\n", address[i]);
	  address[i] = MemorySetupPte(address[i]);
	  //printf("\nPTE for page # %d = %x\n", i, address[i]);
	}
      //printf("\nfinished allocating for page # : %d\n", i);
    }
  ...
  /*
  //user stack - Page number 1023
  //printf("\nallocating for user stack..\n");
  if ((pcb->pagetable[1023] = MemoryAllocPage()) == MEM_FAIL) //page could not be allocated 
    {
      printf("\nFATAL ERROR : Page could not be allocated for user stack.\n");
      exitsim();
    }
  else //page could be allocated 
    {
      pcb->pagetable[1023] = MemorySetupPte(pcb->pagetable[1023]);
    }
  //printf("\nfinished allocating user stack PTE.\n");
  //printf("\nPTE for user stack : %x\n", pcb->pagetable[1023]); 
  */
  ...
  for (j = 0; j < 256; j++)
      ...
      if (l2[j].inuse == 0)
	{
	  ...
	  //printf("\nGetting the l2+j value : %x\n", &(l2[j]));
	  //printf("\nl2[j].table: %x\n", l2[j].table);
	  //pcb->pagetable[3] = &(l2[j]); //the address
	  pcb->pagetable[3] = l2[j].table;
	  break;
	}
    }
  address = pcb->pagetable[3];
  //printf("\nAddress at p[3]: %x\n", address);
  //printf("\nBefore allocating page for user stack...\n");
  //printf("\naddress[255]: %x\n", address[255]);
  if ((address[255] = MemoryAllocPage()) == MEM_FAIL) //page could not be allocated 
    {
      //printf("\nFATAL ERROR : Page could not be allocated.\n");
      exitsim();
    }
    ...
    {
      //printf("\nBefore calling setupPTE() function value = %x\n", address[255]);
      address[255] = MemorySetupPte(address[255]);
      //printf("\nPTE for page of user stack = %x\n", address[255]);
    }
  //printf("\nfinished allocating user stack PTE.\n");
  //printf("\nPTE for user stack : %x\n", pcb->pagetable[1023]); 

  
  stackframe = (uint32 *) (pcb->sysStackArea + 0xffc); //last address of the sysStack PTE - made it 4 byte format
  //printf("\nprinting the system stack frame address : %x\n", (uint32) stackframe); //this is working fine. stackframe will itself have value in hex but will be represented as a 32-bit integer
  ...
  //----------------------------------------------------------------------
  //printf("\nright before PTBASE...\n");
  stackframe[PROCESS_STACK_PTBASE] = (uint32) (& (pcb->pagetable[0])); //remain the same
  //printf("\nPTE Base Address : %x\n", stackframe[PROCESS_STACK_PTBASE]);
  stackframe[PROCESS_STACK_PTSIZE] = MEM_L1TABLE_SIZE; //remain the same
  //printf("\nPT Size: %d\n", stackframe[PROCESS_STACK_PTSIZE]);
  stackframe[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM
				      | (MEM_L2FIELD_FIRST_BITNUM << 16)); //just copied from previous lab -> ask TA what this means -> for L1 PT, they are same
  //printf("\nFinished stack frame shit.\n");
  ...
  if (isUser) {
    ...
    while ((n = ProcessGetFromFile (fd, buf, &addr, sizeof (buf))) > 0) {
      ...
      //printf("\nbefore MemoryCopySystemToUser().\n");
      MemoryCopySystemToUser (pcb, buf, (char *)(addr - n), n);
      //printf("\nfinished MemoryCopySystemToUser().\n");
    }
    //printf("\nout of ProcessGetFromFile().\n");
    FsClose (fd);
    //printf("\nbefore stackframe[stack_isr]\n");
    ...
    //----------------------------------------------------------------------
    printf("\nMaximum Virtual Address: %x\n", MEM_MAX_VIRTUAL_ADDRESS);
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = MEM_MAX_VIRTUAL_ADDRESS - 0x3; //making it 32-word aligned 
    printf("\nstackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
...
void main (int argc, char *argv[])
{
  ...
  //printf("\nBefore all initializations done in process.c...\n");
  ...
  //printf("\nBefore calling processmodule init().\n");
  
  ProcessModuleInit ();
  //printf("\nAfter calling processmodule init().\n");
  ...
  //printf("\nAfter all initializations done in process.c...\n");
  ...
  dbprintf ('i', "Set timer quantum to %d, about to run first process.\n",
	    processQuantum);
  printf("\nbefore the interupt in fork().\n");
  intrreturn ();
  printf("\noutside the interupt in fork().\n");
