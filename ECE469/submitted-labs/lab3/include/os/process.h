...
//#define PRCOESS_STATUS_YIELD    0x
...
typedef struct PCB {
  ...
  int yield; //new addition to the code for q5. this is the yield flag.
  int sleep_start; //if in sleep when the sleep starts in jiffies
  int sleep_end; //cleep_start + sleep_time in jiffies

  //int suspend_called; //flag to check if suspend() process has been called or not
} PCB;
