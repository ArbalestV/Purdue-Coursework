...
PCB *idlePCB; //this is the PCB of the Idle process that runs when there are no processes in the run queue and there are no autowake processes in the wait queue
...
void ProcessModuleInit () {
  ...
  //Since at the beginning, there are no processes running, we set the icdle processes' PCB to null 
  idlePCB = NULL;
...
void ProcessSchedule () {
  ...
  //for ltsched
  //PCB *pcb = NULL;
  int rand; //the random number generated 
  int total_tickets;
  //Link * l = NULL;

  PCB * tmp;
  int cumulative, prev_cumulative;


  int wake_up; //check if there is any process to be awoken from the wait queue
  ...
  currentPCB->process_end_time = ClkGetCurJiffies();
  ...
  if (AQueueEmpty(&runQueue)) {
    if (!AQueueEmpty(&waitQueue)) {
      printf("no runnable processes, but there are sleeping processes waiting!\n");
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
        pcb = AQueueObject(l);
        printf("Sleeping process %d: ", i++); printf("PID = %d\n", (int)(pcb - pcbs));
	if (pcb->sleep_end <= ClkGetCurJiffies())
	  { //that means this process needs to be woken up now
	    wake_up = 1;
	    //call the wakeupprocess() routine on this process
	    ProcessWakeup(pcb); //this process will now be put on the run queue.
	    //break;
	    l = AQueueNext(l);
	  }
	else 
	  {
	    l = AQueueNext(l); //otherwise keep iterating
	  }
      }
      
      currentPCB->process_end_time = ClkGetCurJiffies();
      currentPCB->process_total_time += (currentPCB->process_end_time - currentPCB->process_start_time);
      if (currentPCB->pinfo == 1)
	{
	  printf( PROCESS_CPUSTATS_FORMAT,GetCurrentPid(), currentPCB->process_total_time, currentPCB->pnice);
	  // PROCESS_CPUSTATS_FORMAT(GetCurrentPid(), currentPCB->process_total_time, currentPCB->pnice)
	}
      
      //at this moment, either there is one process that is woken up in the run queue or there is no process that needs to be woken up and nothing in the run queue
      if (wake_up == 0)
	{
	  printf("we are going to idle\n");
	  //nothing to be woken up, and nothing on the run queue, then run the idle process
	  if (idlePCB == NULL)
	    {
	      ProcessFork(ProcessIdle, 0, -1, 0, NULL, 0); //if the idle process hasnt been generated, then generate it for the first time
	    }
	  else
	    {
	      currentPCB = idlePCB; //run the idle process
	      //exitsim();
	      return;
	    }
	}    
    }
    if ( (AQueueEmpty(&runQueue)) && (AQueueEmpty(&waitQueue)))
      {
	//that means the run queue and the wait queue are both empty, which means there are no more processes to run hence stop the simulation
	printf ("No runnable processes - exiting!\n");
	exitsim ();	// NEVER RETURNS
      }
  }
  if(currentPCB == idlePCB && AQueueEmpty(&runQueue)){
    //printf("pointers are the same, %d",time++);
    return;}
  ...
  //now if the process was yielding, then just set the yield flag back to 0
  currentPCB->yield = 0; //RR is fairly easy wrt yielding

  //for q3
  //set pcb finish
  // if (currentPCB->process_start_time) 
  //{
  //currentPCB->process_end_time = ClkGetCurJiffies();
      currentPCB->process_total_time += (currentPCB->process_end_time - currentPCB->process_start_time);
      if (currentPCB->pinfo == 1)
  ...
  //lab3
  //currentPCB->process_start_time = ClkGetCurJiffies();
  ...
  //printf("status of the run queue %d with jiffies %d\n", AQueueEmpty(&runQueue), ClkGetCurJiffies());
  ...
  if (AQueueEmpty(&runQueue)) {
    ...
    if (!AQueueEmpty(&waitQueue)) {
      printf(", but there are sleeping processes waiting!\n");
      l = AQueueFirst(&waitQueue);
      while (l != NULL) {
        pcb = AQueueObject(l);
        printf("Sleeping process %d: ", i++); printf("PID = %d\n", (int)(pcb - pcbs));
	printf("currnet jiffies are %d\n", ClkGetCurJiffies());
	if (pcb->sleep_end <= ClkGetCurJiffies())
	  { //that means this process needs to be woken up now
	    wake_up = 1;
	    //call the wakeupprocess() routine on this process
      ...
      if (wake_up == 0)
	{
	  ...
	  if (idlePCB == NULL)
	    {
	      printf("we are creating the idle process...\n");
	      ProcessFork(ProcessIdle, 0, -1, 0, NULL, 0); //if the idle process hasnt been generated, then generate it for the first time
	      //currentPCB = idlePCB;
	      //printf("we got out of the fork\n");
	      //printf("the pcb is 0x%x, idle pcb is 0x%x\n", (int)currentPCB, (int)idlePCB);
	    }
	  else
	    {
	      printf("we are re-running the idle process...\n");
	      currentPCB = idlePCB; //run the idle process
	      //exitsim();
	      return;
	    }
  ...
  ///--------------------------------------------END OF SAME AS ROUND ROBIN---------------------------------------------------------------
  if(currentPCB == idlePCB && AQueueEmpty(&runQueue)){
    //printf("pointers are the same, %d",time++);
    return;}
  //printf("this code is now different from RR\n");
  //this is where we will check if the process for most of the time quantum or not, ie at least 8 jiffies. If not, then we will know it has been sleeping.
  time_ran = currentPCB->process_end_time - currentPCB->process_start_time;
  if (time_ran >= 8 && currentPCB->yield == 0) //not an I/O process
    { //decrease the number of tickets for the process by 1
      if (currentPCB->pnice == 1)
	{
	  currentPCB->pnice = 1;
	}
      else {
	currentPCB->pnice = currentPCB->pnice - 1;
      }
    }
  else if (currentPCB->yield == 0){ //it is an I/O process thats been sleeping
  ...
  while (l != NULL)
    {
      // printf("l is not null for assigning tickets\n\n");
      tmp = AQueueObject(l);
      total_tickets = total_tickets + tmp->pnice;
      total_processes++;
      l = AQueueNext(l);
    }
  //printf("\nThe total number of tickets: %d\n", total_tickets);
  //printf("\nThe Total Number of processes in the run queue: %d\n", total_processes);
  //now we have gotten the total number of tickets issued
  rand = random() % total_tickets; //now we get the random number that will signify which process to schedule next
  // printf("\nvalue of the random number = %d\n", rand);
  l = AQueueFirst(&runQueue);
  while (l != NULL)
    {
      prev_cumulative = cumulative;
      tmp = AQueueObject(l);
      cumulative = cumulative + tmp->pnice;
      if (rand >= prev_cumulative && rand <= cumulative)
	{ //found the correct process 
	  pcb = (PCB *)AQueueObject(l); //get the pcb
	  //now remove the link l from the queue
	  /*if (AQueueRemove(l) == QUEUE_FAILURE)
	    {
	      dbprintf("\nSomehow not being able to remove the link from queue.\n");
	      }*/
  ...
  //currentPCB->process_end_time = ClkGetCurJiffies();
  //printf("assign current pcb runtime\n");
  currentPCB->process_total_time += (currentPCB->process_end_time - currentPCB->process_start_time);
  if (currentPCB->pinfo == 1)
    {
      printf( PROCESS_CPUSTATS_FORMAT,GetCurrentPid(), currentPCB->process_total_time, currentPCB->pnice);
  ...
  // Move the front of the queue to the end.  The running process was the one in front.
  // printf("right before aqueue after");
  AQueueMoveAfter(&runQueue, AQueueLast(&runQueue), AQueueFirst(&runQueue)); //the current process is now at the last
  //printf("we did Aqueuemoveafter\n\n\n");
  //  currentPCB->yield = 0; //setting the yield flag back to 0 if it was 1
  //if (currentPCB != idlePCB){
    currentPCB = pcb;
    printf("\nThe chosen process PID %d\n", GetCurrentPid());
    //}
...
void ProcessSuspend (PCB *suspend) {
  ...
  //pcb->suspend_called = 1; //since suspend has been called
  ...
  //pcb->suspend_called = 0; //since process suspend has ended
}
...
void ProcessWakeup (PCB *wakeup) {
  ...
  printf("we are waking up %d\n", GetCurrentPid());
  printf("we are waking up at jiffy %d\n", ClkGetCurJiffies());
...
int ProcessFork (VoidFunc func, uint32 param, int pnice, int pinfo,char *name, int isUser) {
  ...
  pcb->npages = 1;
  pcb->pinfo = pinfo;
  //pcb->pnice = pnice; //make sure of the range
  if (pnice >= 19 && func != ProcessIdle)
    {
      pcb->pnice = 19;
    }
  else if (pnice <= 1 && func != ProcessIdle)
    {
      pcb->pnice = 1;
    }
  else
  ...
  RestoreIntrs (intrs); //moved up here

  //code for forking the idle process
  if ( (func == ProcessIdle) && (idlePCB == NULL)) //basically the first time it is being called, set the global PCB value for the idle process
    {
      printf("\nInside the forking process for processIdle...\n");
      idlePCB = pcb;
      currentPCB = pcb;
      return (pcb - pcbs);
    }
  //end of added code for forking the idle process
...
void process_create(char *name, ...)
{
  ...
  //ProcessFork(0, (uint32)allargs, 0, 0, name, 1); //w/o q3
  ProcessFork(0, (uint32)allargs, 1, 1, name, 1); //for q3
}
...
void ProcessUserSleep(int seconds) {
  // Your code here
  currentPCB->sleep_start = ClkGetCurJiffies(); //when the process goes to sleep
  currentPCB->sleep_end = currentPCB->sleep_start + (seconds / ClkGetResolution()); //this is when the process needs to be awoken by
  printf("sending %d to sleep, ", GetCurrentPid());
  printf("sleeping until %d jiffies, ", currentPCB->sleep_end);
  printf("sleep start jiffy = %d\n", currentPCB->sleep_start);
  ProcessSuspend(currentPCB); //call suspend on the current PCB process
}
...
void ProcessYield() {
  // Your code here
  //just set the yield flag to 1 here
  printf("we are yeilding our time... from process %d\n", GetCurrentPid());
  currentPCB->yield = 1;
  //  ProcessSchedule(); //call the process scheduler
}

void ProcessIdle() { //this is the idle function that is run with the PCB of ProcessFork() from ProcessSchedule when there are no processes on the run queue and no autowake processes on the wait queue
