...
et_fh = open("output.txt", 'w') #output file for encrypted output
f_d = open("decrypted.txt", 'w') #output file for storing decrypted text
...
def RSA_decrypt(block): #decryption in RSA function
...
pt_fh = open("message.txt", 'r') #plain text file open
...
pt_fh = open("output.txt", 'r') #encrypted text file open
...
while cur_iter != tot_iter:
    ...
    #print "Count: ", cur_iter + 1
    #print block
    RSA_decrypt(block) #decrypt the block
    cur_iter = cur_iter + 1
