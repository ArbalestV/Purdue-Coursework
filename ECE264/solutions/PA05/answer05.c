...
char * * readString(char * filename, int * numString)
{
  ...
  char oneLine[MAXIMUM_LENGTH];
  int numline = 0; //the total number of lines in file
  while(fgets(oneLine, MAXIMUM_LENGTH, fp) != NULL)
    {
      numline++; //increment number of lines
    }
  *numString = numline;
  fseek(fp, 0, SEEK_SET);
  char * * array;
  array = malloc(sizeof(char *) * numline);
  int length; 
  int i;
  for (i = 0; i < numline; i++)
    {
      fgets(oneLine, MAXIMUM_LENGTH, fp);
      length = strlen(oneLine);
      array[i] = malloc(sizeof(char) * (length + 1));
      strcpy(array[i],oneLine);
    }
  fclose(fp);
  return array;
}
