...
int * readIntegers(const char * filename, int * numberOfIntegers)
{
  ...
  int val;
  int num = 0; //the total number of integers in file
  while(fscanf(fp, "%d", &val) == 1) //when number of integers read equals 1
    {
      num++; //increment number of integers variable "num"
    }
  *numberOfIntegers = num;
  fseek(fp, 0, SEEK_SET);
  int * array = malloc(sizeof(int) * num); //integer array to be returned, size of array is proportional to number of integers read from file
...
void sortHelper(int * arr, int low, int high)
{
  ...
  if (low < high)
    {
      pivot = low; //the first element is chosen as the pivot
      i = low;
      j = high;
      while (i < j)
	{
	  while (arr[i] < arr[pivot] && i <= high)
	    {
	      i++; //when you are making the array for elements smaller than pivot, repeat this step 
	    }
	  while (arr[j] > arr[pivot] && j >= low)
	    {
	      j--; //when you are making the array the elements larger than pivot, repeat this step
	    }
	  if (i < j)
	    {
	      temp = arr[i];
      ...
      //now jth position holds the pivot element
      temp = arr[j];
      arr[j] = arr[pivot];
      arr[pivot] = temp;
      //now, repeat the sorting technique for the two types of array with respect to the pivot (smaller list & larger list)
      sortHelper(arr, low, j - 1); //list smaller than pivot
      sortHelper(arr, j + 1, high); //list larger than pivot
    }
}
