...
SparseNode * SparseArray_insert ( SparseNode * array, int index, int value )
{
  ...
  if (index == array -> index)
    {//found the node, now simply replace the old value
      array -> value = value;
      return array;
    }
  if (index < (array -> index))
...
{ //the minimum index will be at the left most child of the lowest base
  while (array -> left != NULL)
    { //while there is still a left child, there is still a smaller index
      array = array -> left;
    }
  return (array -> index);
}
...
{ //the maximum index will be at the right most child of the lowest base
  while (array -> right != NULL)
    { //while there is still a right child, there is still a larger index
      array = array -> right;
    }
  return (array -> index);
}
...
SparseNode * SparseArray_remove ( SparseNode * array, int index )
{
  ...
  /* if index is the same as array -> index */
  ...
  /* su is node's immediate successor */
  /* swap their values and indexes*/
...
SparseNode * SparseArray_copy(SparseNode * array)
{
  ...
  if (array == NULL) //an empty tree
    {
      return NULL;
    }
  array2 = SparseNode_create(array -> index, array -> value); //create the root of the copied tree
...
SparseNode * mergehelper(SparseNode * cp1, SparseNode * array_2)
{
  if (array_2 == NULL)
    {
      return cp1;
    }
  cp1 = mergehelper(cp1, array_2 -> left);
  cp1 = mergehelper(cp1, array_2 -> right);
  SparseNode * p = SparseArray_getNode(cp1, array_2 -> index);
  if (p != NULL)
    {
      p -> value = (p -> value) + (array_2 -> value);
      if ((p -> value) == 0)
	{
	  cp1 = SparseArray_remove(cp1, p -> index);
	  return cp1;
	}
      return cp1;
    }
    ...
    {
      cp1 = SparseArray_insert(cp1, array_2 -> index, array_2 -> value);
      return cp1;
    }
  //return mergedcopy;
}
...
{
  SparseNode * cp1 = SparseArray_copy(array_1);
  /*if (array_2 == NULL)
    {
      return cp1;
    }*/
  //call mergehelper
  cp1 = mergehelper(cp1, array_2);
  return cp1;
}
