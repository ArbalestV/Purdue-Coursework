...
struct Image * loadImage(const char* filename)
{
  ...
  if (header->magic_bits != ECE264_IMAGE_MAGIC_BITS)
    {
      fclose(fp);
      free(header);
      printf("Magic bits do not match the defined constant.\n");
      return NULL;
    }
  if ((header->width < 0) || (header->width == 0))
    {
      fclose(fp);
      free(header);
      printf("Image width must be greater than 0.\n");
      return NULL;
    }
  if ((header->height < 0) || (header->height == 0))
    {
      fclose(fp);
      free(header);
      printf("Image height must be greater than 0.\n");
      return NULL;
    }

  if ((header->comment_len < 0) || (header->comment_len == 0))
    {
      fclose(fp);
      free(header);
      printf("Comment line length must be greater than 0.\n");
      return NULL;
    }
  ...
  //printf("1");
  if(fread(image->comment, sizeof(char), header->comment_len, fp) != 1)
    {//printf("Failed to open file '%s'\n.", filename);
      free(header);
      fclose(fp);
      free(image->comment);
  ...
  //printf("3");
  if(fread(image->data, sizeof(uint8_t), header->width * header->height, fp)== 0)
    {//printf("Failed to open file '%s'\n.", filename);
      free(header);
      fclose(fp);
      freeImage(image);
