# encoding: utf-8
# module gtk._gtk
# from /usr/lib64/python2.6/site-packages/gtk-2.0/gtk/_gtk.so
# by generator 1.136
# no doc

# imports
import atk as __atk
import gio as __gio
import gobject as __gobject
import gobject._gobject as __gobject__gobject


class CellEditable(__gobject.GInterface):
    # no doc
    @classmethod
    def do_editing_done(cls, *args, **kwargs): # real signature unknown
        pass

    @classmethod
    def do_remove_widget(cls, *args, **kwargs): # real signature unknown
        pass

    @classmethod
    def do_start_editing(cls, *args, **kwargs): # real signature unknown
        pass

    def editing_done(self, *args, **kwargs): # real signature unknown
        pass

    def remove_widget(self, *args, **kwargs): # real signature unknown
        pass

    def start_editing(self, *args, **kwargs): # real signature unknown
        pass

    def __init__(self, *args, **kwargs): # real signature unknown
        pass

    __gtype__ = None # (!) real value is ''


